...
#         {"id":3,"name":"Developing PRoject"},
#     ]
   q=request.GET.get('q') if request.GET.get('q') != None else '' # This condition is for listing room on home page when there is no query in the url 
...
def room(request,pk):
   ...
   room_messages=room.message_set.all() # is a common way to retrrive all messages within that room
...
def loginPage(request):
   page='login'   # this is for Condition and using both register and login code in one page.
...
def userProfile(request,pk):
   ...
   room= user.room_set.all() # is a common way to retrive all rooms related to this user
   room_messages= user.message_set.all()# Is a common way to retrieve all messages relate to this User.
   topics=Topic.objects.all()
